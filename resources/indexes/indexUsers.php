...
if(php_sapi_name() == "cli") {
    ...
    /** @var UserIndexer $userIndexer */
    $userIndexer = $container->get(UserIndexer::class);
    ...
    if (!empty($options['index_name'])) {
        ...
        $userIndexer->setIndexName($indexName);
    } else {
        $indexName = $userIndexer->getIndexName();
    }
    ...
    // count the number of users to be indexed
    $countSql = <<<sql
...
sql;
    ...
    echo "INDEXING USERS ($total total)..." . PHP_EOL;

    // read the users from database (by chunks)
    $sql = <<<sql
        SELECT id, display_name, email, created_on, updated_on
        FROM users
        WHERE TRUE
        {$sinceSql}
        {$idFilterSql}
...
sql;
    ...
    while(true) {
        ...
        foreach ($rows as $row) {
            ...
            if ($options['force-reindex'] || !$userIndexer->isIndexed((int)$row['id'], $lastChangeTS)) {
            try {

                $userIndexer->indexUser($row['id'], $row['display_name'], $row['email']);

            } catch (\Throwable $e) {
                $message = "INDEXING FAILED FOR USER {$row['id']} / {$row['email']}";
