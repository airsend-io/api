...
class MuteCommand implements CommandInterface
{
    ...
    {
        try {
            $this->channelOps->muteChannel($this->user, $this->channel, true);
        } catch (ChannelOpException $e) {
            throw new SlashCommandException(400, $e->getMessage());
        }

        return ['message' => I18n::get('messages.channel_mute_success', ['channel' => $this->channel->getName()])];
    }

    public function getUiSignature()
    {
        return $this->defaultUiSignature('mute', false, ChannelUser::CHANNEL_USER_ROLE_VIEWER);
    }
}